...
import ScriptureTooltip from "./ScriptureTooltip";
import { ScriptureRefPlugin } from "@/lib/scripture/ScriptureRefPlugin";
...
interface NoteEditorProps {
  ...
  noteKey: string;
  noteType: "verse" | "chapter" | "passage" | "sermon" | "meta" | "book";
...
export default function NoteEditor({
  ...
  // Scripture reference tooltip state
  ...
  return (
    <div
      ...
      onMouseOver={(e) => {
        const el = (e.target as HTMLElement).closest("[data-scripture-ref]") as HTMLElement | null;
        if (!el) return;
      ...
      onMouseLeave={() => {
        leaveTimer.current = setTimeout(() => setTooltipRef(null), 150);
      }}
    >
      ...
      {tooltipRef && tooltipPos && (
        <ScriptureTooltip
          osisRef={tooltipRef}
          anchorPos={tooltipPos}
          onCancelLeave={() => {
            if (leaveTimer.current) clearTimeout(leaveTimer.current);
          }}
