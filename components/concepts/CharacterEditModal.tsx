...
/**
 * The same name/color/corpus editor the chapter reading view's Characters
 * sidebar opens on double-click — surfaced here as a standalone modal so
 * it's also reachable from Manage Lists, which has no live chapter to host
 * it inline. Mirrors TagEditModal.tsx; lemma editing is intentionally left
 * out for the same reason as there — adding a lemma re-scans and re-links
 * every matching word in a specific text source/chapter, which only makes
 * sense from a page that already has a chapter loaded.
 */
export default function CharacterEditModal({
  ...
  useEffect(() => {
    let cancelled = false;
    Promise.all([
      fetch(`/api/characters/by-name?name=${encodeURIComponent(characterName)}`).then((r) => r.json()),
      fetch("/api/book-groupings").then((r) => r.json()),
    ]).then(([charsRes, groupingsRes]) => {
      if (cancelled) return;
      setChars(charsRes.characters ?? []);
  ...
  async function handleCreateGrouping(name: string, books: string[]): Promise<BookGrouping> {
    const res = await fetch("/api/book-groupings", {
      ...
      body: JSON.stringify({ name, books, features: ["characters"] }),
    });
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b" style={{ borderColor: "var(--border)" }}>
          <h2 className="text-sm font-semibold truncate" style={{ color: "var(--foreground)" }}>
            Edit &ldquo;{characterName}&rdquo;
          </h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-4 space-y-3">
          ...
          {!loading && chars?.length === 0 && (
            <p className="text-sm" style={{ color: "var(--text-muted)" }}>Character not found.</p>
          )}
          {!loading && (chars?.length ?? 0) > 1 && (
            <p className="text-xs" style={{ color: "var(--text-muted)" }}>
              This name is used by {chars!.length} separate characters across different books — edit each one below.
            </p>
          )}
          {!loading && chars?.map((character) => (
            <CharacterRowEditor
              key={character.id}
              character={character}
...
function CharacterRowEditor({
  ...
  const initialCorpus: CorpusAssignment = {
    mode: (character.corpusType as CorpusAssignment["mode"]) ?? "book",
    groupingId: character.corpusGroupingId ?? null,
    chapter: character.corpusChapter ?? null,
    passageId: character.corpusPassageId ?? null,
  };
  ...
  const [savedSnapshot, setSavedSnapshot] = useState({ name: character.name, color: character.color, corpus: initialCorpus });
