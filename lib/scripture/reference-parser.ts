 ...
 * Handles: "Gen 1", "Gen 1:1", "Gen 1:1-3", "Gen 1:1-2:3"
 *
...
export function parseScriptureRefs(text: string): ScriptureMatch[] {
  ...
  while ((match = SCRIPTURE_REGEX.exec(text)) !== null) {
    ...
    const endChapter =
      endChapterStr !== undefined ? parseInt(endChapterStr, 10) : undefined;
    const endVerse =
      endVerseStr !== undefined ? parseInt(endVerseStr, 10) : undefined;
    ...
    if (verse === undefined) {
      ...
      osisRef = `${osis}.${chapter}.${verse}-${endBook}.${endCh}.${endVerse}`;
    }
